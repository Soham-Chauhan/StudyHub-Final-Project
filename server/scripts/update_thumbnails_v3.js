...
const updateThumbnailsWithRealImages = async () => {
    try {
        ...
        const imageMapping = {
            ...
            'game': '1552824702-3f1a2b3c4d5e',
            'unity': '1552824702-3f1a2b3c4d5e'
        };

        const courses = await Course.find({});
        console.log(`Updating ${courses.length} courses with high-quality technical images...`);

        for (let i = 0; i < courses.length; i++) {
            const course = courses[i];
            const courseNameLower = course.courseName.toLowerCase();
            
            let photoId = '1516321318423-f06f85e504b3'; // Default generic tech image
            ...
            await course.save();
            console.log(`  - Updated [${course.courseName}] with relevant image.`);
        }

        console.log("High-quality thumbnail update complete!");
        mongoose.disconnect();
    } catch (error) {
...
updateThumbnailsWithRealImages();
